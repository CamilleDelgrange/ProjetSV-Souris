...
void CellHandler::setBlood(TypeBloodCell typeBloodCell) //donner une valeur à cellBlood_ si elle est différente de nullptr
{
    if(!hasBlood()) {
        ...
        organ_->updateRepresentationAt(positionGrille_);
    }
}
...
{
    cellECM_->update(sf::seconds(getAppConfig().simulation_fixed_step));
    cellLiver_->update(sf::seconds(getAppConfig().simulation_fixed_step));
    cellBlood_->update(sf::seconds(getAppConfig().simulation_fixed_step));
    if(cellLiver_->cellLiverIsDead())
    {
        delete cellLiver_;
        cellLiver_ = nullptr;
    }
