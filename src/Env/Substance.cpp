...
double Substance::operator [] (const SubstanceId n) const
{
    switch(n) {
    case VGEF :
        return m_total_concentration*m_fract_vgef;
    case GLUCOSE :
        return m_total_concentration*m_fract_glucose;
    case BROMOPYRUVATE :
        return m_total_concentration*m_fract_bromopyruvate;
    default :
        throw std::invalid_argument("Bad substance index");
    }
...
void Substance::uptakeOnGradient(double fraction, Substance& receiver, SubstanceId id)
{
    ...
    switch(id) {
    case GLUCOSE :
        temp = Substance(0, fraction*(*this)[id], 0);
        break;
    case BROMOPYRUVATE :
        temp = Substance(0,0, fraction*(*this)[id]);
        break;
    case VGEF :
        temp = Substance(fraction*(*this)[id], 0, 0);
        break;
    }
    receiver+=temp;
    (*this) -= temp;
